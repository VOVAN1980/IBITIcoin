...
describe("StakingModule – full integration via IBITIcoin proxy", function () {
  ...
  beforeEach(async function () {
    ...
    // 1) Mocks for FeeManager, UserStatusManager, BridgeManager
    ...
    ibiti = await IBITICoinCF.deploy(
      ...
      statusMgr.target,
      bridgeMgr.target,
      ethers.ZeroAddress,     // stakingModule (to bind next)
    ...
    // 5) Configure modules (nftCount = 1 on full-duration unstake)
    await discount.connect(deployer).setDAOModule(staking.target);
    await staking.connect(deployer).setTreasury(treasury.address);
    await staking.connect(deployer).setRewardConfig(1, 1, 1);

    // 6) Seed treasury & approve module
  ...
  it("stake and unstake on-time: principal + reward and 1 NFT", async function () {
    ...
    // One NFT minted on full-duration unstake
