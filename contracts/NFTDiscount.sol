...
contract NFTDiscount is ERC721URIStorage, Ownable, Pausable, ReentrancyGuard {
    ...
    /// @dev Разрешаем вызывать только назначенному оператору
    modifier onlyOperator() {
        require(msg.sender == discountOperator, "NFTDiscount: not operator");
        _;
    }
    ...
    external onlyOwner whenNotPaused
  {
   ...
   // --- Internal common mint logic ---
        ...
        whenNotPaused
    {
       ...
       /// @notice Чеканит уровень Pandora — только владелец, saleManager, DAO или staking
    function mintPandora(address to, string memory tokenURI)
        public
        whenNotPaused
     {
        // разрешаем mint-ить Pandora не только owner’у, но и оператору + модулям
        require(
            msg.sender == owner()          ||
            msg.sender == discountOperator ||
            msg.sender == daoModule        ||
            msg.sender == stakingModule,
            "NFTDiscount: not authorized for Pandora"
        );
        require(!mintedURIs[tokenURI], "URI already used");
    ...
    /**
     * @notice Публичная (для owner-only) функция, которая вызывает внутренний _burn.
