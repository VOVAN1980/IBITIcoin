...
contract StakingModule is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant MAX_NFT_REWARD    = 20;
     ...
     address public allowedCaller;        // e.g. IBITIcoin
    ...
    /// @notice Emitted when allowed caller address is updated
    event AllowedCallerSet(address indexed caller);
    ...
    function setAllowedCaller(address _caller) external onlyOwner whenNotPaused {
        require(_caller != address(0), "Zero caller");
        allowedCaller = _caller;
        emit AllowedCallerSet(_caller);
    }
    ...
    function stakeTokensFor(address staker, uint256 amount, uint256 duration) external whenNotPaused nonReentrant {
        require(msg.sender == staker || msg.sender == address(token) || msg.sender == allowedCaller, "Only staker or token");
     ...
     function unstakeTokensFor(address staker, uint256 index) public whenNotPaused nonReentrant {
    require(msg.sender == staker || msg.sender == address(token) || msg.sender == allowedCaller, "Only staker or token");
    require(treasury != address(0), "Treasury not set");
    ...
    token.safeTransfer(staker, payout);
      ...
      // 3) После переводов — пробуем выдать NFT-награды, но не откатываем при ошибке
      for (uint256 i = 0; i < nftCnt; ++i) {
          // Формируем URI как в _mintRewardNFT
          ...
          // Внешний вызов mint у NFTDiscount теперь в try/catch
          try nftDiscount.mintJackpot(staker, rd.discountPercent, uri) {
              // NFT успешно выдано
          } catch {
              // Фиксируем неудачу (не удалось выдать NFT staker'у)
