 ...
 *
 * ⛑  Контракт теперь **не требует аргументов в конструкторе** —
 *     адрес токена можно выставить позже одним вызовом `setIBIToken`.
 */
...
contract UserStatusManager is Ownable, Pausable {
    ...
    /*───────────────  owner-функции  ───────────────*/
    /// Разово задаём адрес IBITI-токена
    ...
    /*-- ручные флаги --*/
    function setVIPOverride(address u, bool f)   external onlyOwner { vipOverrides[u]   = f; emit VIPOverride(u,f);   }
    function setWhaleOverride(address u, bool f) external onlyOwner { whaleOverrides[u] = f; emit WhaleOverride(u,f); }
    function flagBot(address u, bool f)          external onlyOwner { flaggedBots[u]    = f; emit BotFlagUpdated(u,f); }
