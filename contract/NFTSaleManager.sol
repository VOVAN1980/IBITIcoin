...
contract NFTSaleManager is Ownable, Pausable, ReentrancyGuard {
    ...
    event TokensUpdated(address ibiti, address usdt);
    event NFTDiscountUpdated(address nft);
        ...
        whenNotPaused
    {
        ...
        function setPaymentTokens(address _ibitiToken, address _usdtToken)
       external onlyOwner whenPaused
    {
       require(_ibitiToken != address(0) && _usdtToken != address(0), "zero");
       ibitiToken = IERC20Metadata(_ibitiToken);
       usdtToken  = IERC20Metadata(_usdtToken);
       emit TokensUpdated(_ibitiToken, _usdtToken);
    }

       function setNFTDiscount(address _nftDiscount)
       external onlyOwner whenPaused
    {
       require(_nftDiscount != address(0), "zero");
       nftDiscount = NFTDiscount(_nftDiscount);
       emit NFTDiscountUpdated(_nftDiscount);
     }
        ...
        returns (uint256)
    {
     ...
     /// @notice Спасти сторонние ERC20, отправленные по ошибке
