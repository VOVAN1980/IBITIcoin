...
contract PhasedTokenSale is Ownable, Pausable, ReentrancyGuard {
    ...
    IERC20Metadata public ibiti;
    IERC20Metadata public usdt;
    uint8  public ibitiDecimals;
    uint8  public usdtDecimals;
    uint256 public usdtMultiplier;    // 10^(usdtDecimals−2)
    uint256 public ibitiDenominator;  // 10^ibitiDecimals
    ...
    uint256 public rewardTokens;  // резерв IBITI для рефералов
        // >>> NEW
    uint256 public refReserve;    // отдельный резерв на рефералов
    uint256 public volReserve;    // отдельный резерв на 10 % объём-бонус
    // <<< END
    ...
    event TokensRewired(address ibiti, address usdt);
    event JackpotAirdropped(address indexed to);
...
/// @param _ibiti      IBITI-token (8 dec)
/// @param _usdt       USDT-token (≥2 dec)
/// @param _refReserve резерв на реф-бонус (100 000×1e8)
/// @param _volReserve резерв на объём-бонус (500 000×1e8)
    constructor(
    address _ibiti,
    address _usdt,
    uint256 _refReserve,
    ...
    rewardAmount = 1 * 10 ** ibitiDecimals;   // 1 IBI за друга

    refReserve   = _refReserve;
    volReserve   = _volReserve;
    rewardTokens = _refReserve;               // зеркало для старого фронта
   }
    ...
    function unpause() external onlyOwner {
     ...
     function _recomputeTokenParams() internal {
    ibitiDecimals = ibiti.decimals();
    usdtDecimals  = usdt.decimals();
    require(usdtDecimals >= 2, "USDT decimals<2");

    // 10^(usdtDecimals − 2) — конвертируем «центы» в минимальные единицы USDT
      usdtMultiplier   = 10 ** uint256(usdtDecimals - 2);
      ibitiDenominator = 10 ** uint256(ibitiDecimals);
     }
    ...
    function addVolumeBonus(uint256 threshold, uint16 bonusPct) external onlyOwner whenNotPaused {
    ...
    function setWhitelist(address who, bool ok) external onlyOwner whenNotPaused {
        whitelist[who] = ok;
        emit WhitelistSet(who, ok);
    }

    /// @notice Установить резерв IBITI для реферальных выплат
    function setRewardReserve(uint256 amount) external onlyOwner whenNotPaused {
        rewardTokens = amount;
        refReserve   = amount;       // синхронизируем оба счётчика
        ...
        onlyOwner
     {
      ...
      function setPaymentTokens(address _ibiti, address _usdt) external onlyOwner whenPaused {
      require(_ibiti != address(0) && _usdt != address(0), "zero token");
      ibiti = IERC20Metadata(_ibiti);
      usdt  = IERC20Metadata(_usdt);
      _recomputeTokenParams();
      // Пересчитать зависимые величины:
      rewardAmount = 1 * 10 ** ibitiDecimals;
      emit TokensRewired(_ibiti, _usdt);
    }
    ...
    function setVolumeDiscount(
        ...
        event AirdropParamsUpdated(uint256 discountPercent, string baseURI);

       function setAirdropParams(uint256 _discountPercent, string calldata _baseURI)
       external onlyOwner whenNotPaused
    {
       require(_discountPercent != 100, "Pandora reserved");
       airdropDiscountPercent = _discountPercent;
       airdropBaseURI = _baseURI;
       emit AirdropParamsUpdated(_discountPercent, _baseURI);
     }
    ...
    function setFallbackPrice(uint256 _priceCents) external onlyOwner whenNotPaused {
        fallbackPrice = _priceCents;
        emit FallbackPriceUpdated(_priceCents);
    }

      function topUpVolumeReserve(uint256 amount) external onlyOwner {
      volReserve += amount;                 // учёт внутри контракта
      ibiti.safeTransferFrom(msg.sender, address(this), amount);  // реальные токены
    }

    /// @notice Основная покупка: IBITI за USDT + бонусы + Jackpot-NFT + пост-фазы
    function buy(uint256 tokenAmount, address referrer) external whenNotPaused nonReentrant {
        ...
        for (uint256 i; i < phases.length; ++i) {
            ...
            // >>> NEW  (списываем из volReserve)
            if (bonus > 0) {
                require(volReserve >= bonus, "volume pool empty");
                volReserve -= bonus;
            }
...
if (
    ...
    // чеканим NFT и получаем его ID
      discountContract.mintJackpot(
      msg.sender,
      airdropDiscountPercent,
      uri
    );
       emit JackpotAirdropped(msg.sender);
    }
